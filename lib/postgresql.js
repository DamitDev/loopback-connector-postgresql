...
/**
 * Get relation name from column
 * @param {String} property The property name
 * @returns {String} Relation name
 */
PostgreSQL.prototype.getRelationNameFromColumn = function(property, model) {
  if (this.isNested(property, model) === 'relation') {
...
/**
 * Get a Relation names from order statement
 * @param {Object} order Filter object
 * @returns {Array} Relation names ['clienPartner']
 */
PostgreSQL.prototype.getOrderByRelationNames = function(order, model) {
  if (!order) {
    return [];
  }
  const self = this;
  ...
  for (let i = 0, n = order.length; i < n; i++) {
    ...
    if (t.length === 1) {
      const relationModel = self.getRelationNameFromColumn(order[i], model);
      if (relationModel) {
        relationModels.push(relationModel);
      }
    } else {
      const relationModel = self.getRelationNameFromColumn(t[0], model);
      if (relationModel) {
        relationModels.push(relationModel);
      }
    }
...
/**
 * Get a Relation names from where statement
 * @param {Object} where Filter object
 * @returns {Array} Relation names ['clienPartner']
 */
PostgreSQL.prototype.getWhereRelationNames = function(where, model) {
  if (!where) {
    return [];
  }
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (let i = 0, n = clauses.length; i < n; i++) {
          relationModels.push(...self.getWhereRelationNames(clauses[i], model));
        }
        continue;
      }
    }
    const relationModel = self.getRelationNameFromColumn(key, model);
    if (relationModel) {
      relationModels.push(relationModel);
    }
  }
...
/**
 * Get a Relation names from where and order statement
 * @param {String} model Model name
 * @param {Object} whereFilter Filter object
 * @param {Object} orderFilter Filter object
 * @returns {Array} Relation names ['clienPartner']
 */
PostgreSQL.prototype.getRelatioNames = function(model, whereFilter, orderFilter) {
  return this.getOrderByRelationNames(orderFilter, model).concat(this.getWhereRelationNames(whereFilter, model));
};

/**
 * Build a SQL JOIN statement
 * @param {String} model Model name
 * @param {Array} relationModels Model names
 * @returns {String} Statement string
 */
PostgreSQL.prototype.getJoinTables = function(model, relationModels) {
  const relations = [...new Set(relationModels)];
  let joinStmnt = '';
  for (const relationName of relations) {
    const relation = this.getModelDefinition(model).model.relations[relationName];
    if (relation.modelThrough) {
      joinStmnt += this.getThroughStatement(relation);
      continue;
    }
    const foreignKey = this.getForeignKey(relation, true);
    const primaryKey = this.getPrimaryKey(relation, true);
    const modelToName = this.getModelName(relation.modelTo);
    const modelFromName = this.getModelName(relation.modelFrom);

    joinStmnt += ` LEFT JOIN ${this.schema(modelToName)}.${this.table(modelToName)} as "${relationName}"
                  ON "${relationName}"."${primaryKey}" = "${this.table(modelFromName)}"."${foreignKey}" `;
  }
  return joinStmnt;
};

PostgreSQL.prototype.getThroughStatement = function(relation) {
  const {modelThrough, modelFrom, modelTo, keyThrough, name} = relation;
  const modelThroughName = this.getModelName(modelThrough);
  const modelFromName = this.getModelName(modelFrom);
  const modelToName = this.getModelName(modelTo);
  const primaryKey = this.getPrimaryKey(relation, true);
  const foreignKey = this.getForeignKey(relation, true);

  const tableThrough = `${this.schema(modelThroughName)}.${this.table(modelThroughName)}`;
  const table = `${this.schema(modelToName)}.${this.table(modelToName)}`;

  return `  LEFT JOIN ${tableThrough} as ${modelThroughName}
            ON ${modelThroughName}.${primaryKey} = ${modelFromName}.${foreignKey}

            LEFT JOIN ${table} as ${name}
            ON ${modelThroughName}.${keyThrough} = ${name}.${foreignKey}
        `;
};
 ...
 */
PostgreSQL.prototype.buildColumnNamesWithTableName = function(model, filter) {
  const fieldsFilter = filter && filter.fields;
  const cols = this.getModelDefinition(model).properties;
  const tableName = `"${this.table(model)}".`;
  if (!cols) {
    return '*';
  }
  const self = this;
  ...
  const names = keys.map(function(c) {
    return tableName + self.columnEscaped(model, c);
  });

  const orderColumn = sanitizeOrder(filter.order);
  if (orderColumn) {
    names.push(orderColumn);
  }
...
const sanitizeOrder = (order) => {
  // default order by Loopback ['id']
  if ((Array.isArray(order) && order[0] === 'id') || order.split('.').length === 1) {
    return false;
  }
  return order.replace(/desc|asc/ig, '');
};
...
PostgreSQL.prototype.buildSelect = function(model, filter) {
  ...
  if (filter && (filter.where || filter.order)) {
    relationNames = this.getRelatioNames(model, filter.where, filter.order);
  }
  let selectStmt;
  if (relationNames.length) {
    selectStmt = new ParameterizedSQL('SELECT DISTINCT ' +
      this.buildColumnNamesWithTableName(model, filter) +
      ' FROM ' + this.tableEscaped(model));

    // Merge join statement
    selectStmt.merge(this.getJoinTables(model, relationNames));

    if (filter) {
      if (filter.where) {
        const whereStmt = this.buildWhere(model, filter.where, true); // true --> appendTableName
    ...
    selectStmt = new ParameterizedSQL('SELECT ' +
 ...
 * @param {Object} loopback model object
 * @returns {String} modelName
 */
PostgreSQL.prototype.getModelName = (model) =>{
  return model.definition.name;
};

/*
 * Get FK from a relation
 * @param {Object} relation object
 * @param {Boolean} If FK's SQL table column name should be return or the Loopack definition
 * @returns {String} foreignKey
 */
PostgreSQL.prototype.getForeignKey = (relation, sql = false) => {
  const {keyFrom, modelFrom} = relation;
  const propertyDefinition = modelFrom.definition.properties[keyFrom];
  // check if there is an explicit deifnition, if not, return the lowercased version as
  // loopback formats
  return sql ? (_.get(propertyDefinition, 'postgresql.columnName') || keyFrom.toLowerCase()) : keyFrom;
};

PostgreSQL.prototype.getPrimaryKey = (relation, sql = false) => {
  const {keyTo, modelTo} = relation;
  const propertyDefinition = modelTo.definition.properties[keyTo];
  return sql ? (_.get(propertyDefinition, 'postgresql.columnName') || keyTo.toLowerCase()) : keyTo;
};

/*
 * Checks if the property has postgresql definition, and if its dataType `json`
 * @param {Object} property
 * @returns {Boolean}
 */
PostgreSQL.prototype.propertyIsJSON = (property) => {
  return (property.postgresql && property.postgresql.dataType === 'json');
};
 ...
 * @param {String} model The model name
 * @returns {String} 'json' of the property contains dots for nested json, 'relation' of the property contains dots for nested relation
 */
PostgreSQL.prototype.isNested = function(property, model) {
  if (property.split('.').length > 1) {
    const key = property.split('.')[0];
    const relation = this.getModelDefinition(model).model.relations[key];
    if (!relation) {
      return '';
    }
    const foreignKey = this.getForeignKey(relation);
    const prop = this.getModelDefinition(model).properties[foreignKey];

    return this.propertyIsJSON(prop) ? 'json' : 'relation';
  } else {
    return '';
  }
};
...
PostgreSQL.prototype.columnEscaped = function(model, property) {
  if (this.isNested(property, model) === 'json') {
  ...
  } else if (this.isNested(property, model) === 'relation') {
    const self = this;
    const relationName = self.getRelationNameFromColumn(property, model);
    const relation = self.getModelDefinition(model).model.relations[relationName];
    return property
      .split('.')
      .map(function(val, idx) {
        return (idx === 0 ? `"${val}"` : self.columnEscaped(relation.modelTo.definition.name, val));
      })
...
PostgreSQL.prototype.buildExpression = function(columnName, operator,
  ...
  switch (operator) {
    case 'eq':
      return new ParameterizedSQL(columnName + '::TEXT = ?',
        [operatorValue]);
    case 'like':
 ...
 */
PostgreSQL.prototype.buildWhere = function(model, where, appendTableName = false) {
  const whereClause = this._buildWhere(model, where, appendTableName);
  if (whereClause.sql) {
 ...
 */
PostgreSQL.prototype._buildWhere = function(model, where, appendTableName = false) {
  ...
  for (const key in where) {
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (let i = 0, n = clauses.length; i < n; i++) {
          const stmtForClause = self._buildWhere(model, clauses[i], appendTableName);
    ...
    let columnName, p;
    if (self.isNested(key, model) === 'relation') {
      columnName = self.columnEscaped(model, key);
      const arr = key.split('.');
      const relation = self.getModelDefinition(model).model.relations[arr[0]];

      p = relation.modelTo.definition.properties[arr[1]];
    } else {
      p = props[key];

      if (p == null && self.isNested(key, model)) {
      ...
      let tableName = '';
      if (appendTableName) {
        tableName = `"${this.table(model)}".`;
      }
      columnName = tableName + self.columnEscaped(model, key);
    }
