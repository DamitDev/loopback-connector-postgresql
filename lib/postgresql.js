...
PostgreSQL.prototype.buildOrderBy = function(model, order) {
  ...
  for (let i = 0, n = order.length; i < n; i++) {
    ...
    if (t.length === 1) {
      clauses.push(self.columnEscaped(model, underscoreize(order[i])));
    } else {
      clauses.push(self.columnEscaped(model, underscoreize(t[0])) + ' ' + t[1]);
    }
  }
...
PostgreSQL.prototype.columnEscaped = function(model, property) {
  if (this.isNested(property, model) === 'json') {
    ...
    return property
      .split('.')
      .map(function(val, idx) { return (idx === 0 ? `"${val}"` : self.columnEscaped(model, val)); })
