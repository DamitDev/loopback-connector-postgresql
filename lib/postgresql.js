...
PostgreSQL.prototype.getJoinTables = function(model, relationModels) {
    ...
    for (const relationName of relations) {
      ...
      let foreignKey = this.getModelDefinition(model).properties[relation.foreignKey].postgresql.columnName;
      joinStmnt += ` LEFT JOIN ${this.schema(relation.model)}.${this.table(relation.model)} as "${relationName}" on "${relationName}"."id" = "${this.table(model)}"."${foreignKey}" `;
    }
    return joinStmnt;
};
