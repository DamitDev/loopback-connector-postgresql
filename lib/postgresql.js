...
var PLACEHOLDER = PostgreSQL.PLACEHOLDER = ParameterizedSQL.PLACEHOLDER;
...
PostgreSQL.prototype.getOrderByRelationNames = function(order, model) {
  ...
  for (let i = 0, n = order.length; i < n; i++) {
    ...
    if (t.length === 1) {
      let relationModel = self.getRelationNameFromColumn(order[i], model);
      if (relationModel) {
        relationModels.push(relationModel);
      }
    } else {
      let relationModel = self.getRelationNameFromColumn(t[0], model);
      if (relationModel) {
        relationModels.push(relationModel);
      }
    }
...
PostgreSQL.prototype.getWhereRelationNames = function(where, model) {
  ...
  for (const key in where) {
    ...
    let relationModel = self.getRelationNameFromColumn(key, model);
    if (relationModel) {
      relationModels.push(relationModel);
    }
  }
 ...
 */
PostgreSQL.prototype.getJoinTables = function(model, relationModels, lang) {
  const relations = [...new Set(relationModels)];
  let joinStmnt = '';
  for (const relationName of relations) {
    const relation = this.getModelDefinition(model).settings.relations[relationName];
    if (relation.type !== 'belongsTo') {
      throw new Error(g.f(`Nested column accepted only belongsTo relationship: ${relationName}`));
    }
    const foreignKey = this.getModelDefinition(model).properties[relation.foreignKey].postgresql.columnName;
    joinStmnt += ` LEFT JOIN ${this.getFromClause(relation.model, lang, false)} as "${relationName}" 
      on "${relationName}"."id" = "${this.table(model)}"."${foreignKey}" `;
  }
  return joinStmnt;
};
 ...
 * @param {string} model Model name
 * @param {number} language id
 * @returns {string} The FROM clause
 */
PostgreSQL.prototype.getFromClause = function(model, lang, needAlias = true) {
  if (lang && this.getModelDefinition(model).settings.translation) {
    const tableName = this.getModelDefinition(model).settings.postgresql.table;
    const alias = (needAlias) ? `as ${tableName}` : '';
    const translationFields = Object.entries(this.getModelDefinition(model).properties)
      .filter(prop => prop[1].translate)
      .map(prop => prop[0]);
    const selectClause = translationFields.map(prop => `COALESCE(x.${prop}, '''missing translation''') as ${prop}`);
    const crosstabValues = translationFields.map(prop => `('${prop}'::text)`);
    const finalResult = translationFields.map(prop => `"${prop}" text`);

    return `(SELECT ${selectClause.join(',')},main.* FROM
              ${this.tableEscaped(model)} as main
            LEFT JOIN
            (SELECT *
            FROM crosstab(
              'select
              t.source_id::text,
              t.property::text,
              t.translation_value::text
            from
              system.translation t
            where
              t.sourcetype = ''${model}'' and 
              t.system_language_id = ''${lang}''
            order by 1,2', $$VALUES ${crosstabValues.join(',')}$$) 
            AS final_result("source_id" text, ${finalResult.join(',')})) AS x 
            ON x.source_id::text = main.id::text) ${alias}`;
  } else {
    return this.tableEscaped(model);
  }
};
...
PostgreSQL.prototype.buildSelect = function(model, filter) {
  ...
  if (relationNames.length) {
    selectStmt = new ParameterizedSQL('SELECT ' +
      ...
      ' FROM ' + this.getFromClause(model, filter.lang));

    // Merge join statement
    selectStmt.merge(this.getJoinTables(model, relationNames, filter.lang));
...
PostgreSQL.prototype.isNested = function(property, model) {
  if (property.split('.').length > 1) {
    ...
    const relation = this.getModelDefinition(model).settings.relations[key];
    if (!relation) {
      return '';
    }
    return (this.getModelDefinition(model).properties[relation.foreignKey].postgresql.dataType === 'json') ? 'json' : 'relation';
  } else {
    return '';
  }
};
...
PostgreSQL.prototype.columnEscaped = function(model, property) {
  if (this.isNested(property, model) === 'json') {
    ...
    const relation = self.getModelDefinition(model).settings.relations[relationName];
    return property
      .split('.')
      .map(function(val, idx) {
        return (idx === 0 ? `"${val}"` : self.columnEscaped(relation.model, val));
      })
...
PostgreSQL.prototype._buildWhere = function(model, where, appendTableName=false) {
  ...
  for (const key in where) {
    ...
    if (self.isNested(key, model) === 'relation') {
      ...
      let arr = key.split('.');
      let relation = self.getModelDefinition(model).settings.relations[arr[0]];
      p = self.getModelDefinition(relation.model).properties[arr[1]];
    } else {
      p = props[key];
